    ...
    <div class="ftr-col">
      ...
      <ul class="hours">
        ${DAY_KEYS.slice().reverse().map(k => {
          const h = s.hours[k] || {};
          return `<li class="${k === DAY_KEYS[new Date().getDay()] ? 'is-today' : ''}"><span>${DAY_AR[k]}</span><b>${h.closed ? 'مغلق' : `${fmtShort(h.open)} – ${fmtShort(h.close)}`}</b></li>`;
        }).join('')}
      </ul>
    </div>
    <div class="ftr-col">
    ...
    <div class="hero-art reveal">
      <div class="hero-card">
        <div class="hero-card-top">
          ...
          <b>${(() => { const h = (s.hours && s.hours[DAY_KEYS[new Date().getDay()]]) || {}; return h.closed ? 'مغلق اليوم' : `${fmtShort(h.open || '09:00')} — ${fmtShort(h.close || '24:00')}`; })()}</b>
        </div>
  ...
  <div class="cinfo reveal">
    ...
    <div class="cbox cbox--static"><span>${icon('clock')}</span><div><b>أوقات العمل</b>
      <ul class="hours hours--dark">
        ${DAY_KEYS.map(k => { const h = s.hours[k] || {}; return `<li class="${k === DAY_KEYS[new Date().getDay()] ? 'is-today' : ''}"><span>${DAY_AR[k]}</span><b>${h.closed ? 'مغلق' : `${fmtShort(h.open)} – ${fmtShort(h.close)}`}</b></li>`; }).join('')}
      </ul>
    </div></div>
  </div>
