...
function layout({ title, desc, body, active = '', data, art, extraHead = '', jsonLd = '', canonical = '', needArt = true }) {
  ...
  const artTag = `<script>window.FS = ${fsObj};</script>` + (needArt ? '<script src="/assets/js/art.js?v=37" defer></script>' : '');
...
<script type="application/ld+json">${JSON.stringify(jsonLdFull).replace(/undefined/g, 'null')}</script>${jsonLd ? `\n<script type="application/ld+json">${jsonLd}</script>` : ''}
${extraHead}
</head>
<body class="${active}">
...
<header class="hdr" id="hdr">
  ...
  <div class="hdr-main">
    <div class="wrap hdr-main-in">
      <button class="hdots" id="btnMore" type="button" aria-label="القائمة والخيارات" aria-expanded="false"><span class="lines3" aria-hidden="true"><i></i><i></i><i></i></span></button>
      <a class="brand" href="/">
        <img class="brand-logo" src="${esc(s.logoUrl || '/assets/logo/logo-lockup-t.png')}" alt="${esc(s.name)}">
      </a>
...
<footer class="ftr">
  ...
  <div class="wrap ftr-grid">
    <div class="ftr-col ftr-brand">
      <span class="flogo"><img src="${esc(s.logoFullUrl || '/assets/logo/logo-lockup-t.png')}" alt="${esc(s.nameFull)}"></span>
...
<script src="/assets/js/app.js?v=37" defer></script>
  <div class="msheet-bd" id="msheetBd"></div>
  <div class="msheet" id="msheet" role="dialog" aria-label="خيارات إضافية">
    <a href="/">${icon('home')} الرئيسية</a>
    <a href="/products">${icon('cart')} المنتجات</a>
    <a href="/services">${icon('wrench')} الخدمات</a>
    <a href="/about">${icon('shield')} من نحن</a>
    <a href="/contact">${icon('phone')} تواصل معنا</a>
    <a href="/products?cat=parts">${icon('part')} قطع الغيار</a>
    <a href="https://wa.me/9665313653" target="_blank" rel="noopener">${icon('wa')} واتساب</a>
    <a href="tel:0531365453">${icon('phone')} اتصال مباشر</a>
  </div>
</body>
</html>`;
}
...
function productCard(p, cat, hasArt = true) {
  ...
  return `<article class="card pcard reveal" data-cat="${esc(p.category)}" data-id="${esc(p.id)}">
    <a class="pcard-media pcard-link" href="/product/${esc(p.id)}" aria-label="${esc(p.name)} — عرض الصور والتفاصيل">
      ...
      <span class="pcard-zoom" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg></span>
    </a>
    <div class="pcard-body">
      ...
      <div class="pcard-foot">
        <div class="price">
          ${p.price > 0 ? `${p.oldPrice && p.oldPrice > p.price ? `<s>${money(p.oldPrice)}</s>` : ''}<b>${money(p.price)}</b><span>ريال</span>` : '<b class="price-call">السعر بالتواصل</b>'}
        </div>
        <div class="pcard-acts">
          ${art && p.price > 0 ? '<button class="btn btn--ghost btn--xs" data-add="' + esc(p.id) + '">' + icon('cart') + ' أضف</button>' : ''}
          <a class="btn btn--wa btn--xs" target="_blank" rel="noopener" href="${waLink(s, `السلام عليكم، أبغى أستفسر عن: ${p.name}${p.price > 0 ? ` (${money(p.price)} ريال)` : ''} — ${s.nameFull}`)}">${icon('wa')} اطلب</a>
        </div>
      </div>
    </div>
  </article>`;
...
<section class="hero">
  ...
  <div class="wrap hero-in">
    ...
    <div class="hero-art reveal">
      <div class="hero-card">
        <div class="hc-status ${(() => { const h = (s.hours && s.hours[DAY_KEYS[riyadhNow().getDay()]]) || {}; return h.closed ? 'is-closed' : 'is-open'; })()}"><i></i>${(() => { const h = (s.hours && s.hours[DAY_KEYS[riyadhNow().getDay()]]) || {}; return h.closed ? 'مغلق الآن · يفتح ' + fmtShort(h.open || '09:00') : 'مفتوح الآن · يغلق ' + fmtShort(h.close || '24:00'); })()}</div>
        <div class="hc-hours">${icon('clock')}<div><b>أوقات العمل اليوم</b><i>${(() => { const h = (s.hours && s.hours[DAY_KEYS[riyadhNow().getDay()]]) || {}; return h.closed ? 'مغلق اليوم' : fmtShort(h.open || '09:00') + ' — ' + fmtShort(h.close || '24:00'); })()}</i></div></div>
        <div class="hc-acts">
          <a class="btn btn--wa btn--lg btn--block" href="${waLink(s)}" target="_blank" rel="noopener">${icon('wa')} اطلب واتساب</a>
          <a class="btn btn--glass btn--lg btn--block" href="tel:${tel(s.phone)}">${icon('phone')} ${esc(s.phoneDisplay)}</a>
        </div>
        <p class="hc-note">رد سريع داخل أوقات العمل · توصيل وتركيب مجاني داخل الرياض</p>
      </div>
    </div>
  </div>
  ${wave('hero-wave')}
  ...
  <div class="wrap">
    <div class="filter-bar">
      <div class="filters" id="filters">
        ...
        ${data.categories.map(c => `<button class="fchip ${cat === c.id ? 'is-on' : ''}" data-f="${esc(c.id)}">${esc(c.short)} <b>${data.products.filter(p => p.category === c.id).length}</b></button>`).join('')}
      </div>
      <div class="sortwrap">
        <label>ترتيب:
          <select id="sortSel">
...
<section class="sec sec--tight">
  <div class="wrap pdetail">
    ...
    <div class="pd-info">
      ...
      <div class="pd-price">
        ${p.price > 0 ? `${p.oldPrice && p.oldPrice > p.price ? `<s>${money(p.oldPrice)} ريال</s>` : ''}<b>${money(p.price)}</b><span>ريال</span>` : '<b>السعر بالتواصل</b>'}
      ...
      <div class="pd-acts">
        ${p.price > 0 ? `<button class="btn btn--gold btn--lg" data-add="${esc(p.id)}">${icon('cart')} أضف لسلة الطلب</button>` : ''}
        <a class="btn btn--wa btn--lg" target="_blank" rel="noopener" href="${waLink(s, `السلام عليكم، أبغى أطلب: ${p.name}${p.price > 0 ? ` (${money(p.price)} ريال)` : ''} — ${s.nameFull}`)}">${icon('wa')} اطلب عبر واتساب</a>
...
<section class="sec sec--tight"><div class="wrap wrap--narrow">
  <div class="prose reveal">
    ...
    <p>انطلقنا من محل واحد بالرياض، ومن وعد واحد ما تغيّر: <b>الجهاز المناسب لموية بيتك</b> — مو الأغلى ولا الأرخص — وتركيب يضبط من أول مرة، وبعد البيع نحن معك بالصيانة والتذكير تلقائيًا.</p>
...
<section class="sec sec--tight"><div class="wrap">
  <div class="locband reveal">
    <span class="loc-ic">${icon('pin')}</span>
    <div class="loc-tx">
      <b>معرضنا بالرياض — السويدي الغربي</b>
      <i>${esc(s.address)}</i>
      ${s.addressNote ? `<em>${esc(s.addressNote)}</em>` : ''}
    </div>
    <div class="loc-acts">
      <a class="btn btn--gold btn--lg" href="${esc(s.maps)}" target="_blank" rel="noopener">${icon('pin')} افتح الاتجاهات</a>
      <a class="btn btn--glass btn--lg" href="tel:${tel(s.phone)}">${icon('phone')} اتصال</a>
    </div>
  </div>
</div></section>`;
