...
html = html.replace('<link rel="stylesheet" href="/assets/css/style.css?v=13">', '<style>\n' + read('public/assets/css/style.css') + '\n</style>');
html = html.replace('<script src="/assets/js/art.js?v=13" defer></script>', '<script>\n' + read('public/assets/js/art.js') + '\n</script>');
html = html.replace('<script src="/assets/js/app.js?v=13" defer></script>', '<script>\n' + read('public/assets/js/app.js') + '\n</script>');
...
for (const f of ['logo-full.svg', 'logo-full-white.svg', 'logo-symbol.svg', 'logo-symbol-mono.svg']) {
...
// صور المنتجات (مجلد كامل)
const prodDir = path.join(ROOT, 'public', 'assets', 'products');
if (fs.existsSync(prodDir)) {
  for (const f of fs.readdirSync(prodDir)) {
    const u = '/assets/products/' + f;
